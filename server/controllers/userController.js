...
export const register = async (req, res) => {
    try {
        ...
        if(!name || !email || !password){
            return res.json({success: false, message: 'Missing Details Please fill All Detail register'})
        }
        ...
        res.cookie('token', token, {httpOnly: true, // Prevent JavaScript to access cookie
...
export const isAuth = async (req, res) => {
    try {
        ...
        console.log(error.message, 'unauthorized request');
        res.json({ success: false, message: error.message });
    }
};
